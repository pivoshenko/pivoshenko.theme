"""Module that contains the script that renders theme files from palette JSON."""  # noqa: EXE002
...
def _render_target_from_template(
    ...
    # Supported forms:
    # - legacy flat: templates/ghostty.conf.jinja -> <output_dir>/ghostty/<theme_name>.conf
    # - nested: templates/ghostty/theme.conf.jinja -> <output_dir>/ghostty/<theme_name>.conf
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--palette",
        required=True,
        help="Path to palette JSON (e.g. morok/palettes/morok.json)",
    )
    parser.add_argument(
        "--templates-dir",
        default=None,
        help="Directory with *.jinja templates (default: <palette>/../../templates)",
    )
    parser.add_argument(
        "--output-dir",
        default=None,
